  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="w-16 h-16 border-4 border-gray-300 dark:border-gray-600 border-t-gray-900 dark:border-t-gray-100 rounded-full animate-spin"></div>
      </div>
    )
  }
          ...
          {formData.acquisition_cost > 0 && formData.current_market_value > 0 && (
            <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Acquisition</div>
                  <div className="font-bold text-gray-900 dark:text-gray-100">
                    {formatCurrency(formData.acquisition_cost, formData.currency)}
                  </div>
                  {formData.currency === 'USD' && (
                    <div className="text-xs text-gray-500 dark:text-gray-400">≈ {formatCurrency(cadEquivAcquisition)} CAD</div>
                  )}
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Valeur marchande</div>
                  <div className="font-bold text-gray-900 dark:text-gray-100">
                    {formatCurrency(formData.current_market_value, formData.currency)}
                  </div>
                  {formData.currency === 'USD' && (
                    <div className="text-xs text-gray-500 dark:text-gray-400">≈ {formatCurrency(cadEquivMarket)} CAD</div>
                  )}
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Appréciation</div>
