...
DROP POLICY IF EXISTS "portfolio_public_read"      ON portfolio_profiles;
DROP POLICY IF EXISTS "portfolio_org_manage"       ON portfolio_profiles;
...
DROP POLICY IF EXISTS "portfolio_items_org_manage"  ON portfolio_items;
DROP POLICY IF EXISTS "portfolio_items_fill_token"  ON portfolio_items;
...
-- Fill token: acces via lien (filtre reel fait cote client sur fill_token)
CREATE POLICY "portfolio_fill_token_update" ON portfolio_profiles
  FOR UPDATE TO anon, authenticated
  USING (true)
  WITH CHECK (true);

-- Lecture via fill_token (pour charger le profil sur /portfolio/fill/[token])
...
-- Fill token items: acces complet via lien de remplissage
...
DROP TRIGGER IF EXISTS portfolio_profiles_updated_at ON portfolio_profiles;
...
DO $$ BEGIN
  RAISE NOTICE 'Migration 174: portfolio_profiles + portfolio_items crees avec RLS (numerotation corrigee).';
END $$;
