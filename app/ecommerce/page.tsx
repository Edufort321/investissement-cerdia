      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
        <div className="relative aspect-[3/4] bg-gray-100">
          {images.length > 0 ? (
            <>
              {!imageError[current] ? (
                <Image src={images[current]} alt={product.name} fill className="object-cover" onError={() => setImageError({...imageError, [current]: true})} unoptimized loader={({ src }) => src} />
              ) : (
              ...
              {images.length > 1 && (
                <>
                  <button onClick={() => setCurrent((current - 1 + images.length) % images.length)} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black bg-opacity-40 text-white rounded-full w-6 h-6 flex items-center justify-center">‹</button>
                  <button onClick={() => setCurrent((current + 1) % images.length)} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black bg-opacity-40 text-white rounded-full w-6 h-6 flex items-center justify-center">›</button>
          ...
          {(hasPriceValue(product.priceCa) || hasPriceValue(product.priceUs)) && (
            <div className="mb-3">
              <p className="font-semibold text-sm text-gray-900">
                {t('indicativePrice')} 
                {hasPriceValue(product.priceCa) && ` ${product.priceCa}$`}
                {hasPriceValue(product.priceCa) && hasPriceValue(product.priceUs) && ' |'}
